...
// ----- BONUS FUNCTIONS (upper, lower, insert, trim) -----
...
// // ----- end of BONUS FUNCTIONS (upper, lower, insert, trim) -----
...
// strcat ->
// START_TEST(strcat_Hello_aboba) {  // ??????? same as below
//   char src[] = " aboba!!!";
//   char res[5 + 10] = "Hello";
...
// START_TEST(strcat_zero_first) {  // ?????????????????????? work on school mac,
//                                  // but not on arm mac!
//   char src[] = " aboba!!!";
//   char res[0 + 10] = "";
//   char expected[0 + 10] = "";
...
START_TEST(strcat_any_letters_with_register) {
  ...
  strcat(
      expected,
      src);  // ?????????????????????? if change with s21_strcat stop working!!
  s21_strcat(res, src);
  ck_assert_str_eq(res, expected);
}
END_TEST
...
START_TEST(sprintf_many_flags_many_ints) {
  ...
  char format[] = "%0.5hi %d %4.13i %013d %d";
  int val = 69;
  ck_assert_int_eq(s21_sprintf(str1, format, val, -10431, 5311, 0, -581813581),
                   sprintf(str2, format, val, -10431, 5311, 0, -581813581));
...
START_TEST(sprintf_many_flags_many_ints2) {
  char str1[BUFF_SIZE];
  char str2[BUFF_SIZE];
...
Suite *suite_insert(void) {
  Suite *s = suite_create("Units");
  TCase *tc = tcase_create("test ");
  // Suite *f = suite_create("TEST 2");
  // TCase *tf = tcase_create("TEST 2");
  ...
  tcase_add_test(tc, sprintf_many_flags_many_ints2);
